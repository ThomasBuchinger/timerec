...
var editTaskCmd = &cobra.Command{
	Use:   "edit NAME",
	Short: "Edit a Job",
	Long: `Jobs can be worked on with only a name. BUT more details are required to save them permanently
...
func EditTaskRun(cmd *cobra.Command, args []string) {
	...
	cli.UpdateJob(args[0], template, title, description, project, taskInProject)
